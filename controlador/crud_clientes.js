...
crud_cliente.leer = (req,res) => {
    conectar.query( 'select id_cliente, nit, nombre, apellidos, direccion, telefono, date_format(fecha_nacimiento, "%d-%m-%Y") as fecha_nacimiento from clientes', (error, results) => {
        if(error){
            throw error;
        }else{
...
crud_cliente.cud = (req,res) => {
    ...
    if(btn_crear){
        conectar.query('insert into clientes set?',{nit:nit,nombre:nombre,apellidos:apellidos,direccion:direccion,telefono:telefono,fecha_nacimiento:fecha_nacimiento},(error,results) => {
            if(error){
                console.log(error);
            }else{
                res.redirect('/');
    ...
    if(btn_actualizar){
        conectar.query('update clientes set? where id_cliente = ?',[{nit:nit,nombre:nombre,apellidos:apellidos,direccion:direccion,telefono:telefono,fecha_nacimiento:fecha_nacimiento},id_cliente],(error,results) => {
            if(error){
                console.log(error);
            }else{
                res.redirect('/');
